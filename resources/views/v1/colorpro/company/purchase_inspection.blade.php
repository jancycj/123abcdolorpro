    ...
    <div class="container pd-5">
        <div class="import-sec-white mg-b-5">
            <div class="d-sm-flex align-items-center justify-content-between ">
                <div>
                    <div >
                        <h5 class="hd">#Purchase Inspection </h5> 
                    </div> 
                </div>
      ...
      <div class="row row-xs">
                                ...
                                <tbody>
                                    <tr v-for="(d,index) in searched_details">
                                        ...
                                        <td class="number-field">
                                            <input autocomplete="off" class="form-control "  v-model="d.accepted_qty"> 
                                        </td class="number-field">
                                        <td class="number-field">
                                            <input autocomplete="off" class="form-control"  v-model="d.cond_accepted_qty"> 
                                        </td class="number-field">
                                        <td class="number-field">
                                            <input autocomplete="off" class="form-control"  v-model="d.rework_qty" @keydown.prevent="getReworkPopup($event, d )" > 
                                        </td>
                                        <td class="number-field">
                                            <input autocomplete="off" class="form-control"  v-model="d.rejected_qty" @keydown.prevent="getRejectPopup($event, d)" > 
                                        </td>
        ...
        <div class="modal fade" id="reworkPopup" tabindex="1" role="dialog" aria-labelledby="exampleModalLabel4"
                ...
                <div class="modal-content tx-14 card">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="card-body card-block">
                            <div class="row">
                                <div class="col-12" @keydown="updateRework($event)" >
                                    <div class="form-group row">
                                        <label class="col-3 form-control-label">rework quantity*</label>
                                        <div class="col-9 input-group">
                                            <input autocomplete="off" class="form-control"  v-model="rework_qty" >
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12" @keydown="updateRework($event)" >
                                    <div class="form-group row">
                                        <label class="col-3 form-control-label">rework reason*</label>
                                        <div class="col-9 input-group">
                                            <textarea name="rework_reason" id="rework_reason" cols="10" rows="10" class="form-control"  v-model="rework_reason"></textarea>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-outline-primary tx-13" @click.prevent="updateRework()">Create</button>
                    </div>
                </div>
            </div>
        </div><!-- modal end -->

        <div class="modal fade" id="rejectPopup" tabindex="1" role="dialog" aria-labelledby="exampleModalLabel4"
  aria-hidden="true" data-backdrop="static">
            <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                ...
                <div class="modal-content tx-14 card">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="card-body card-block">
                            <div class="row">
                                <div class="col-12" @keydown="updateReject($event)" >
                                    <div class="form-group row">
                                        <label class="col-3 form-control-label">reject quantity*</label>
                                        <div class="col-9 input-group">
                                            <input autocomplete="off" class="form-control"  v-model="rejected_qty" >
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12" @keydown="updateReject($event)" >
                                    <div class="form-group row">
                                        <label class="col-3 form-control-label">reject reason*</label>
                                        <div class="col-9 input-group">
                                            <textarea name="reject_reason" id="rejected_reason" cols="10" rows="10" class="form-control"  v-model="rejected_reason"></textarea>
                                        </div>
                                    </div>
                                </div>
...
where orh.company_id = {{DocNo::companyId()}} AND IFNULL(orcd.accepted_quantity,0) + IFNULL(orcd.conditionally_accepted_quantity,0)
 + IFNULL(orcd.rework_quantity,0)
 + IFNULL(orcd.rejected_quantity,0)
< IFNULL(orcd.recieved_quantity,0) AND co.name like "%$vrbl%"
order by orh.mir_no DESC limit 10`;
   ...
   methods: {
         ...
         getReworkPopup: function(evnt, d){
             ...
             this.rework_reason = d.rework_reason;
            $("#reworkPopup").modal('toggle');
           
         },
         updateRework : function(event){

            var vm = this;
            
            if(event.code == 'Enter' ){
                vm.mir_details.forEach( it => {
                    if(it.id === vm.selected_id){
                        it.rework_qty = vm.rework_qty;
                        it.rework_reason = vm.rework_reason;
                    }
                })
                $("#reworkPopup").modal('toggle');
                vm.selected_id = '';
        ...
        getRejectPopup: function(evnt, d){
             this.selected_id = d.id;
             this.rejected_qty = d.rejected_qty;
             this.rejected_reason = d.rejected_reason;
            $("#rejectPopup").modal('toggle');
           
         },
         updateReject : function(event){
            ...
            if(event.code == 'Enter' ){
                vm.mir_details.forEach( it => {
                    if(it.id === vm.selected_id){
                        it.rejected_qty = vm.rejected_qty;
                        it.rejected_reason = vm.rejected_reason;
                    }
                })
                $("#rejectPopup").modal('toggle');
                vm.selected_id = '';
        ...
        isValidMir : function(){
            ...
            this.mir_details.forEach(function(obj){
                ...
                let total = obj.mir_quantity;
                let accepted_total = parseFloat(obj.accepted_qty?obj.accepted_qty:0) +
                                        parseFloat(obj.cond_accepted_qty?obj.cond_accepted_qty:0) + 
                                        parseFloat(obj.rework_qty?obj.rework_qty:0) +
                                        parseFloat(obj.rejected_qty?obj.rejected_qty:0);

                if( accepted_total > total ){
                    alert('can not accept more than recieved for '+ obj.part_no)
                    flag =  false;
                }
        ...
        mirEdit : function(evnt){
            ...
            var vm = this;
            vm.update_flag = true;
            axios.get('/company/purchase/getInspectionData/'+evnt.id).then((response) => {
        ...
        save_mir:function() {
         ...
         update_mir:function() {
            ...
            axios.post( '/company/purchase/inspection/',vm.mir).then(response => {
                // this.mir.mir_no = response.data.mir_no;
                // this.mir.mir_date = response.data.mir_date;
